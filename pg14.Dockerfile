...
RUN dnf -y install --allowerasing dnf-utils curl git make gcc && \
    ...
    dnf -y install postgresql${PG_MAJOR}-server postgresql${PG_MAJOR}-contrib edb-pg${PG_MAJOR}-pg-failover-slots1 && \
    dnf clean all
...
COPY --chown=postgres:postgres primary/postgresql.conf ${PGDATA}/
...
CMD ["/usr/pgsql-14/bin/postgres", "-D", "/var/lib/pgsql/14/data"]